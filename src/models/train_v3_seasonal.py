"""
train_v3_seasonal.py -- Seasonal V3 Training Script (Phase 10)

Trains the RVRAttentionLSTM_V3 model specifically on Fog Months (Oct-Mar) 
to reduce distributional shift from summer/monsoon noise.
Strictly uses an alphabetical target sorting standard.
"""
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(ROOT / "logs" / "phase10_v3_seasonal_training.log"),
    ],
)
...
CONFIG = {
    ...
    "run_name":        "Residual-Attention-LSTM-V3-Seasonal",
...
def prepare_multi_dataloaders():
    ...
    # Enforce alphabetical sorting of targets
    ...
    # -- Split & Seasonal Filter (Oct - March)
    fog_months = [10, 11, 12, 1, 2, 3]
    
    train_df = df[(df.index.year <= 2023) & (df.index.month.isin(fog_months))]
    val_df   = df[(df.index.year == 2024) & (df.index.month.isin(fog_months))]
    test_df  = df[(df.index.year == 2025) & (df.index.month.isin(fog_months))]

    logger.info(f"Split sizes (Seasonal: Oct-Mar) -> Train: {len(train_df):,} | Val: {len(val_df):,} | Test: {len(test_df):,}")

    if len(train_df) == 0:
        raise ValueError("Seasonal filter resulted in 0 training samples. Check dataset timestamps.")

    # -- Scale (Using separate seasonal scalers to avoid leakage from non-fog distribution)
    os.makedirs(CONFIG["scaler_dir"] + "_seasonal", exist_ok=True)
    scaler_X = StandardScaler()
    scaler_X.fit(train_df[feature_cols])
    joblib.dump(scaler_X, os.path.join(CONFIG["scaler_dir"] + "_seasonal", "scaler_X.pkl"))

    scaler_y = StandardScaler()
    scaler_y.fit(train_df[target_cols])
    joblib.dump(scaler_y, os.path.join(CONFIG["scaler_dir"] + "_seasonal", "scaler_y.pkl"))
...
def main(use_wandb=True):
    ...
    logger.info(f"Targeting: {device} for Seasonal Training")
    ...
    logger.info(f"Seasonal V3 Model Params: {total_params:,}")
    ...
    patience_counter = 0
    best_ckpt = os.path.join(CONFIG["checkpoint_dir"], "best_lstm_v3_seasonal.pt")
    ...
    for epoch in range(1, CONFIG["epochs"] + 1):
        ...
        if val_loss < best_val_loss:
            ...
            patience_counter = 0
            torch.save({"epoch": epoch, "model_state": model.state_dict(), "val_mae": val_mae, "config": CONFIG, "seasonal": True}, best_ckpt)
            logger.info(f"  [SAVED] Best Seasonal model")
        else:
            patience_counter += 1
    ...
    # Final Evaluation
    logger.info("Running Best Seasonal Model on Seasonal Test Set...")
    ...
    logger.info(f"FINAL SEASONAL TEST MAE: {test_mae:.2f}m")
